function test
    ...
    fprintf(fileID, 'Delay Factor\n');
    fprintf(fileID, 'x; y; x estimate; y estimate; sync error; TDOA error; position error; delay factir\n');
    delayFactor = 0.001;
    for i = 1:11
        [coords, syncError, tdoaError, coordError] = simulation(0.5, 0.25, 65, 48000, 15000, delayFactor);
        fprintf(fileID, '0.5; 0.25; %f; %f;', coords(1), coords(2));
        posError = sqrt(coordError(1)^2+coordError(2)^2);
        fprintf(fileID, '%f; %f; %f; %f\n', mean(syncError), mean(tdoaError), posError, delayFactor);
        delayFactor = delayFactor * 2;
    end
    
    fprintf(fileID, 'Calibration Signal Position Error\n');
    fprintf(fileID, 'x; y; x estimate; y estimate; sync error; TDOA error; position error; calibration signal position error\n');
    calPosError = 0.005;
    for i = 1:20
    ...
    fprintf(fileID, 'Mic Position Error\n');
    fprintf(fileID, 'x; y; x estimate; y estimate; sync error; TDOA error; position error; mic position error\n');
    micPosError = 0.005;
    for i = 1:20
